...
In a multi-repository workspace, make the pointer name the repository, path or subtree, active Logical Context, and source revision when the target is revision-sensitive. Never infer those from the current directory, and never use a host-absolute path as the only locator.
...
- The **environment** is a source of truth too — build and toolchain configuration, repository layout, checked-in verification commands, and tool `--help` output — and a document that restates it is a **cache**: a copy of a lookup, earning its load only when the lookup is expensive. In a multi-repository workspace, say which repository and revision owns the fact. Cache what the agent cannot find by looking: the unwritten convention, the reason behind a choice, the gotcha no config confesses. Leave cheap lookups to their authoritative source, where they cannot go stale.
