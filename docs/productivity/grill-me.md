...
`grill-me` always invokes the model-reachable `grilling` primitive. If the harness cannot invoke that skill, the session reports `BLOCKED`; it does not improvise a reduced interview inline.
...
- **A workspace to align against** — [grill-with-docs](https://aihero.dev/skills-grill-with-docs). The same interview, but [stateful](https://www.aihero.dev/ai-coding-dictionary/stateful): it reads the relevant repositories and keeps what it learns in the active Logical Context artifacts and ADRs.
...
Yes. Add this to your harness-level agent instructions, such as `AGENTS.md` or `CLAUDE.md`:
...
That portability is the whole difference from [grill-with-docs](https://aihero.dev/skills-grill-with-docs), which runs the same interview but reads the relevant workspace repositories and records what it learns in the active Logical Context artifacts and ADRs. Both sit on the [grilling](https://aihero.dev/skills-grilling) primitive; `grill-me` is the user-invoked front door that carries nothing with it.
